...
class ThemeSettingController extends Controller
{
    ...
    public function themeCustomize(Request $request,$id)
    {
        if(Auth::user()->isAbleTo('theme manage'))
        {
            ...
            return view('theme_customize.index',compact('id','theme_json'));
        }
        else
        {
    ...
    public function customize_theme($id, $slug, $sub_slug)
    {
        if(Auth::user()->isAbleTo('theme edit')) 
        {
            if (!empty($id)) {
                if ($slug) {
                    ...
                    $themeSetting   = ThemeSetting::where('business_id',getActiveBusiness())->where('theme', $id)->pluck('value', 'key');

                    return view('theme_customize.edit', compact('id', 'slug', 'sub_slug', 'theme_json', 'themeSetting'));
                } else {
                    return abort(404);
                }
            } else {
    ...
    public function customize_theme_update($id, Request $request)
    {       
        if(Auth::user()->isAbleTo('theme edit'))
        {
            ...
            foreach ($data as $key => $value) {
                ThemeSetting::updateOrCreate(['theme' => $id, 'key' => $key, 'business_id' =>getActiveBusiness(), 'created_by' => creatorId()] , ['value' => $value]);
            }
