    ...
    public function destroy($business_id)
    {
        if (Auth::user()->isAbleTo('business delete')) {
            ...
            if ($business && $business->created_by == $objUser->id) {
                ...
                return redirect()->route('dashboard')->with('errors', __("You can't delete Business! because your other businesses are disabled "));
            } else {
                return redirect()->route('dashboard')->with('errors', __("You can't delete Business!"));
            }
        } else {
            return redirect()->back()->with('error', __('Permission denied.'));
        }
